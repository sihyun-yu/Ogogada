...
>
> Safety: There were some tradeoffs between these two prototypes. In some aspects, paper prototype was better. Note that we needed to connect all buttons by hotspot if we want some action by clicking certain area. Therefore, if user clicked the area looks to be done something but not connected by any hotspot, clearly it happens nothing and some users get confused about this. However, since human computer in paper prototype knows everything about any buttons, this may not happen. However, in some aspects, paper prototype is worse than lo-fi prototype. For instance, human computer can make a mistake like calculating the sum of prices of menu, but hotspot we connected never makes mistake. 
